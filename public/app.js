...
// Tombol chat admin umum (tanpa produk spesifik)
const generalWaLink = `https://wa.me/${ADMIN_WHATSAPP}?text=${encodeURIComponent('Halo Admin Khaliji Shop, saya mau tanya-tanya 🙂')}`;
navWaBtn.addEventListener('click', () => window.open(generalWaLink, '_blank'));
floatingWaBtn.href = generalWaLink;
...
db.collection('settings').doc('store').get().then(doc => {
  ...
  console.error(err);
  // Kalau gagal cek status (misal koneksi bermasalah), tetap tampilkan toko
  // supaya pembeli tidak terjebak di layar kosong.
  appView.classList.remove('hidden');
  init();
});
...
function init() {
  db.collection('products').orderBy('createdAt', 'desc').get().then(snap => {
    allProducts = snap.docs
      ...
      .filter(p => !p.sold); // sembunyikan produk yang sudah terjual
    renderCategories();
  }).catch(err => {
...
function openCategory(catName) {
  ...
  if (items.length === 0) {
    ...
    items.forEach(p => {
      ...
      card.innerHTML = `
        ${p.imageUrl
          ? `<img class="thumb" src="${p.imageUrl}" alt="${escapeHtml(p.title)}">`
